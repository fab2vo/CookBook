 ...
 * input : pwd , all fields of recipe but photo and date et message= optionmessage pour nouvelle recette
 ...
 */
 ...
 if (! preg_match($pattern, $title_req)) exitfail("Error in title ".$title_req);
 $pattern="/[A-Za-zÀ-ÿ #-_!?.,(\$£€)%*&:+0-9]{0,254}/";  
 $source_req=addslashes($_POST['source']);
 if (! preg_match($pattern, $source_req)) exitfail("Error in source ".$source_req);
 $sourceurl_req=trim($_POST['sourceurl']);
 if (! preg_match('/^(http|https|ftp):\/\/([A-Z0-9][A-Z0-9_-]*(?:.[A-Z0-9][A-Z0-9_-]*)+):?(d+)?\/?/i', $sourceurl_req)) exitfail("Error in source url ".$sourceurl_req);
 $nbpers_req=$_POST['nbpers'];
 if (($nbpers_req<1)||($nbpers_req>12)) exitfail("Error in nb pers ".$nbpers_req);
 $pattern=$pat_etape; 
 $etape01_req=addslashes($_POST['etape01']); 
 if (! preg_match($pattern, $etape01_req)) exitfail("Error in step ".$etape01_req);
 $etape02_req=addslashes($_POST['etape02']);
 if (! preg_match($pattern, $etape02_req)) exitfail("Error in step ".$etape02_req); 
 $etape03_req=addslashes($_POST['etape03']); 
  if (! preg_match($pattern, $etape03_req)) exitfail("Error in step ".$etape03_req);
 $etape04_req=addslashes($_POST['etape04']);
 if (! preg_match($pattern, $etape04_req)) exitfail("Error in step ".$etape04_req); 
 $etape05_req=addslashes($_POST['etape05']); 
  if (! preg_match($pattern, $etape05_req)) exitfail("Error in step ".$etape05_req);
 $etape06_req=addslashes($_POST['etape06']); 
 if (! preg_match($pattern, $etape06_req)) exitfail("Error in step ".$etape06_req); 
 $etape07_req=addslashes($_POST['etape07']); 
  if (! preg_match($pattern, $etape07_req)) exitfail("Error in step ".$etape07_req);
 $etape08_req=addslashes($_POST['etape08']); 
  if (! preg_match($pattern, $etape08_req)) exitfail("Error in step ".$etape08_req);
 $etape09_req=addslashes($_POST['etape09']);  
 if (! preg_match($pattern, $etape09_req)) exitfail("Error in step ".$etape09_req); 
 $season_req=trim($_POST['season']);
 if (!in_array($season_req, array('SUMMER','WINTER','ALLYEAR'))) exitfail("Error in season ".$season_req); 
 $difficulty_req=trim($_POST['difficulty']);
 if (!in_array( $difficulty_req, array('QUICK','EASY','ELABORATE','SOPHISTICATED','UNDEFINED'))) exitfail("Error in difficulty ". $difficulty_req); 
 $type_req=trim($_POST['type']);
 if (!in_array( $type_req, array('APERITIF','STARTER','MAIN','DESSERT','SIDE','OTHER'))) exitfail("Error in type ". $type_req); 
 $pattern=$pat_ing;
 $ing01_req=addslashes($_POST['ing01']);
 if (! preg_match($pattern, $ing01_req)) exitfail("Error in ingredient ".$ing01_req);
 $ing02_req=addslashes($_POST['ing02']);
 if (! preg_match($pattern, $ing02_req)) exitfail("Error in ingredient ".$ing02_req);
 $ing03_req=addslashes($_POST['ing03']);
 if (! preg_match($pattern, $ing03_req)) exitfail("Error in ingredient ".$ing03_req);
 $ing04_req=addslashes($_POST['ing04']);
 if (! preg_match($pattern, $ing04_req)) exitfail("Error in ingredient ".$ing04_req);
 $ing05_req=addslashes($_POST['ing05']);
 if (! preg_match($pattern, $ing05_req)) exitfail("Error in ingredient ".$ing05_req);
 $ing06_req=addslashes($_POST['ing06']);
 if (! preg_match($pattern, $ing06_req)) exitfail("Error in ingredient ".$ing06_req);
 $ing07_req=addslashes($_POST['ing07']);
 if (! preg_match($pattern, $ing07_req)) exitfail("Error in ingredient ".$ing07_req);
 $ing08_req=addslashes($_POST['ing08']);
 if (! preg_match($pattern, $ing08_req)) exitfail("Error in ingredient ".$ing08_req);
 $ing09_req=addslashes($_POST['ing09']);
 if (! preg_match($pattern, $ing09_req)) exitfail("Error in ingredient ".$ing09_req);
 $ing10_req=addslashes($_POST['ing10']);
 if (! preg_match($pattern, $ing10_req)) exitfail("Error in ingredient ".$ing10_req);
 $ing11_req=addslashes($_POST['ing11']);
 if (! preg_match($pattern, $ing11_req)) exitfail("Error in ingredient ".$ing11_req);
 $ing12_req=addslashes($_POST['ing12']);
 if (! preg_match($pattern, $ing12_req)) exitfail("Error in ingredient ".$ing12_req);
 $ing13_req=addslashes($_POST['ing13']);
 if (! preg_match($pattern, $ing13_req)) exitfail("Error in ingredient ".$ing13_req);
 $ing14_req=addslashes($_POST['ing14']);
 if (! preg_match($pattern, $ing14_req)) exitfail("Error in ingredient ".$ing14_req);
 $ing15_req=addslashes($_POST['ing15']);
 if (! preg_match($pattern, $ing15_req)) exitfail("Error in ingredient ".$ing15_req);
 $date_req=$_POST['date'];
 if (! preg_match($pat_date, $date_req)) exitfail("Error in date format ".$date_req);
 // personnalisation message possible et pwd
 if (isset($_POST['message'])) $message_req=addslashes($_POST['message']);
 else  $message_req="Nouvelle recette dans la famille";
 ...
 if (!$result) exitfail("Recipe query empty not found");
 ...
 if (!$result) exitfail("MySQL query fail");
 ...
 if ($row_count==0) {
	...
	if ($row_member_count==0) exitfail("No family found - error") ;
	while ($ligne=mysqli_fetch_assoc($result)) {
	/* --- insert for each member */
		extract($ligne);
		if ($id_user==$iduser_req) {$status_req='Visible';} else {$status_req='Submitted';}
		$sql2=" INSERT INTO `cookbooks`(`id_user`, `id_recipe`, `status`, `message`, `id_from`) 
				VALUES ('$id_user','$idrecipe_req','$status_req','$message_req','$iduser_req')";
		debugtrace($sql2);
		mysqli_query($cxn,'SET NAMES UTF8');
		$result2 = mysqli_query($cxn,$sql2);
		if (!$result2) exitfail("MySQL query fail");
		debugtrace("Recipe inserted");
	}
 }
