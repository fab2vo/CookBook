 ...
 */
 ...
 debugtrace("Id_recipe :".$idrecipe_req);
 if (! preg_match($pat_uuid, $iduser_req)) exitfail("Error in Id user ".$iduser_req);
 debugtrace("Id_recipe :".$iduser_req);
 if (! preg_match($pat_date, $date_req)) exitfail("Error in date format ".$date_req);
 if ( $comment_req=="...") exitfail("Error in comment : ...");
 if ( $comment_req=="") exitfail("Error in comment : empty ");
 if (! preg_match($pat_etape, $comment_req)) exitfail("Error in comment with pattern :".$comment_req);
